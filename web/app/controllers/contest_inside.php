<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if (isset($_POST["user_paper_download"]) && isSuperUser($myUser)) {
			echo "<style>h1,h2,h3{font-weight:300}.card{border: 1px solid #888;padding: 8px 12px;}.bdg{background-color:#1780db20;padding: 3px 6px;}</style><center><h1>{$contest["id"]} {$contest["name"]} 选手答卷</h1></center>";
			...
			echo "<h2 id='top'>目录</h2><ul>";
			foreach ($all_registrants as $r) {
				echo "<li><a href='#player-{$r["username"]}'>{$r["username"]}</a></li>";
			}
			echo "</ul>";
			foreach ($all_registrants as $r) {
				echo "<hr/>";
				echo "<h2 id='player-{$r["username"]}'>{$r["username"]}的成绩单</h2>";
				$pl = queryContestUserProblemList($contest, array("username" => $r["username"]));
				$cnt = 0;
				$pcnt = 1;
...
?>
<p>
<b><?= $chinese_count[$cnt]; ?>、<?= $pf["problem_type"] === NULL ? "全部题型" : $problem_type[$pf["problem_type"]]; ?>(共<?= $pf["problem_count"]; ?>题，每题<?= $pf["problem_score"]; ?>分，满分<?= intval($pf["problem_count"]) * intval($pf["problem_score"]); ?>分)</b>
</p>
<?php
					foreach ($pl[$cnt] as $p) {
						$sql = "SELECT *, score * {$pf["problem_score"]} / 100 as s from submissions where contest_id={$contest["id"]} and problem_id={$p["id"]} and submitter = '{$r["username"]}' ORDER BY id DESC LIMIT 1";
						//echo $sql;
						$submission = DB::selectFirst($sql);
						$score = intval($submission["s"]);
						$sql = "SELECT statement from problems_contents where id={$p["id"]}";
						$res = DB::selectFirst($sql);
						$pc = $res["statement"];
						$pc = str_replace("\n-","<br>-",$pc);
?>
	<ol start="<?= $pcnt; ?>">
	<li><p><b>题目标题：</b>#<?= $p["id"]; ?> <?= $p["title"]; ?></p><p><b>题目内容：</b><div class="card"><?= $pc ?></div></p><p><b>选手答案：</b><?php echoSubmissionContent($submission, getProblemSubmissionRequirement($p)); ?></p><p><b>选手得分：</b><div class="bdg"><?= $score ?> / <?= $pf["problem_score"] ?></div></p></li>
</ol>
<?php
						$pcnt ++;
					}
...
EOD;
	...
	function echoMyProblems() {
		...
		uojIncludeView('contest-my-problems', [
			'contest' => $contest,
			'problem_filters' => $problem_filters,
			'problem_type' => $problem_type,
			'problem_list' => $problem_list_res
		]);
	}
...
EOD;
		...
		if (Cookie::get('show_all_submissions') !== null && $canSeeAll) {
			echoSubmissionsList("contest_id = {$contest['id']}", 'order by id desc', $cfg, $myUser);
		} else {
			echoSubmissionsList("submitter = '{$myUser['username']}' and contest_id = {$contest['id']}", 'order by id desc', $cfg, $myUser);
		}
	}
