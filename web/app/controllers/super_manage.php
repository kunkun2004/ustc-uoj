<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if (isset($_POST["user_list"])) {
			$user_list_json = $_POST["user_list"];
...
<div class="row">
	...
	<div class="col-sm-9">
		<?php if ($cur_tab === 'users'): ?>
			<h3>账号导入</h3>
			<form id="form-user-import" class="form-horizontal" method="post">
				<div id="div-user-list" class="form-group">
					<label for="input-user-list" class="col-sm-2 control-label">用户信息列表</label>
					<textarea class="form-control" id="input-user-list" name="user_list" placeholder="请输入导入的用户信息，一行一个。&#13;每行输入用户名和密码空格隔开&#13;例如：user1 passwd&#13;user2 passwd"></textarea>
			...
			<div class="table-responsive" id="table-import-user"></div>
	...
	let importedUserStatus = [];
	let rawPasswords = [];
	function showUserImportTable() {
		let tableHTML = `<table class="table table-bordered table-hover table-striped table-text-center">
			...
			<tbody>`;
		for (let i = 0; i < importedUserList.length; i++) {
			tableHTML += `<tr>
				<td>${importedUserList[i].username}</td>
				<td>${rawPasswords[i]}</td>
				<td>${importedUserList[i].email}</td>
				<td>${importedUserStatus[i]}</td>
			</tr>`;
		}
		tableHTML += `</tbody>
		</table>`;
		$("#table-import-user").html(tableHTML);
	}
	function submitUserImport() {
		importedUserList = [];
		importedUserStatus = [];
		const salt = "<?= getPasswordClientSalt() ?>";
		
		const lines = $("#input-user-list").val().split("\n");
		for (const line of lines) {
			...
			const password = parts?.[1] || generateRandomPassword();
			const email = parts?.[2] || `${generateRandomPassword(8)}@${generateRandomPassword(4)}.com`;
			
			importedUserList.push({
				username,
			...
			importedUserStatus.push("提交中");
		}
		$.post("/super-manage", {
			user_list : JSON.stringify(importedUserList)
		}, (msg) => {
			importedUserStatus = msg.split(",");
			while (importedUserStatus.length < importedUserList.length) {
				importedUserStatus.push("失败");
			}
			showUserImportTable();
			$("#input-user-list").val("");
		});
	...
	$(document).ready(function() {
		$('#form-user-import').submit(function(e) {
			submitUserImport();
			return false;
		});
	});
