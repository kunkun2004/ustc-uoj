...
$.fn.source_code_form_group = function (name, text, langs_options_html) {
	return this.each(function () {
		...
		var input_language =
			$('<div class="code_selection" />').html($('<select id="' + input_language_id + '" name="' + input_language_name + '" id="codeSelection"/>')
				.html(langs_options_html)).append($('<div class="editor_name">代码编辑器(text/x-csrc)</div>'));
		...
		var div_editor =
			$('<div id="' + div_editor_id + '" class="col-sm-12"/>')
				.append(input_editor)
				.append($('<div class="col-sm-2 offset-sm-3 radio" style="display: none" />')
					.append($('<label/>')
						.append(input_upload_type_editor)
						.append(' '+uojLocale('editor::upload by editor'))
					)
				)
				.append($('<div class="checkbox text-right" style="display: none" />')
					.append($('<label />')
